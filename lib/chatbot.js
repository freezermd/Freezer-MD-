...
`.trim();
    ...
    for (const api of API_ENDPOINTS) {
        try {
            ...
            if (!response.ok) {
                console.log(
                    `${api.name} failed with status ${response.status}`
                );
                continue;
            }
            ...
            const cleanedResponse = result
                ...
                .replace(/thinks/g, '🤔')
                .replace(/sleeps/g, '😴')
                .replace(/google/gi, 'Freezer')
                .replace(/popkid bot/gi, 'Freezer')
...
async function handleChatbotResponse(
    ...
    try {
        ...
        if (
            ...
            console.error(
                'Session error in chatbot - skipping error response'
            );
        ...
        try {
            ...
            console.error(
                'Failed to send chatbot error message:',
                sendError.message
            );
        }
    }
